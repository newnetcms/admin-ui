$(document).ready(function () {
    ...
    function strSlug($title, $separator = '-') {
        if (!$title) return '';
        ...
        for (let $key in charsArray) {
            $title = $title.replace(new RegExp(`${[].concat(charsArray[$key]).join('|')}`, 'gi'), $key);
        }
        $title = $title.replace(/[^\x20-\x7E]/giu, '');
        // Convert all dashes/underscores into separator
        const $flip = $separator == '-' ? '_' : '-';
        $title = $title.replace(new RegExp('[' + pregQuote($flip) + ']+', 'g'), $separator);
        // Remove all characters that are not the separator, letters, numbers, or whitespace.
        $title = $title.replace(new RegExp('[^' + pregQuote($separator) + 'a-z0-9\\s]+', 'gi'), $separator);

        // Replace all separator characters and whitespace by a single separator
        $title = $title.replace(new RegExp('[' + pregQuote($separator) + '\\s]+', 'g'), $separator);
    ...
    $("input.slug").each(function () {
        let $slug = $(this);
        let data = $slug.data();
        let $slugFrom = $(data.slugFrom);
        let prefix = data.slugPrefix;

        function generateSlug() {
            let slug = strSlug($slugFrom.val());
            if (prefix) {
                slug = prefix + '/' + slug;
            }
